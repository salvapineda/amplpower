...
def solve_obbt(ps, obj="minimize_Pfa_1", opf_type="dc", connectivity="off", solver="gurobi", options="", relax="all"):
    ...
    relax (str or int): Relaxation strategy for binary variables:
    ...
    Returns:
    tuple: Solve status and the value of the objective function or None if infeasible.
    """
    direction, variable, line_index = obj.split("_")
    ps.create_model(opf_type, connectivity)
    ps.ampl.eval(f"fix status[{line_index}]:=0;")
    ps.ampl.eval(f"{direction} newbound: {variable}[{line_index}];")

    if relax == "all":
        ps.ampl.eval("option relax_integrality 1;")
    elif isinstance(relax, int) and relax > 0:
        relaxed_lines = close_lines(ps, int(line_index), level=relax)
    ...
    if solve_status == "solved":
        return solve_status, ps.ampl.get_objective("newbound").value()
    elif solve_status == "infeasible":
        return solve_status, None
    else:
        return solve_status, ps.ampl.get_value("newbound.bestbound")


def update_bounds(ps, relax="all"):
    """Update the bounds for each line in the power system.
    Parameters:
    ps (PowerSystem): An instance of the PowerSystem class.
    relax (str or int): Relaxation strategy for binary variables:
        - "all": Relax all binary variables (default).
    ...
    for lin_index in range(ps.nlin):
        # Maximize Pfa
        solve_status, new_bound = solve_obbt(
            ps, obj=f"maximize_Pfa_{lin_index}", solver="gurobi", options="outlev=0 threads=1 timelimit=1 mip:bestbound=1", relax=relax
        )
        ...
        # Minimize Pfa
        solve_status, new_bound = solve_obbt(
            ps, obj=f"minimize_Pfa_{lin_index}", solver="gurobi", options="outlev=0 threads=1 timelimit=1 mip:bestbound=1", relax=relax
        )
        if solve_status == "infeasible":
            print(f"Infeasible for line {lin_index}")
        else:
